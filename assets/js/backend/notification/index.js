  ...
  class ListIndexNotificationBackendEKP {
    ...
    initSweetAlert(id) {
      swal({
        ...
        if (value) {
          //get AJAX data
          Cloud.trashNotification.with({ ids: id,  _csrf: window.SAILS_LOCALS._csrf }).protocol('jQuery').exec((err, responseBody, responseObjLikeJqXHR) => {
            if (err) {
              console.log(err);
              return;
    ...
    initPushAlert(id) {
      swal({
        ...
        if (value) {
          //get AJAX data
          Cloud.pushNotification.with({ id: id,  _csrf: window.SAILS_LOCALS._csrf }).protocol('jQuery').exec((err, responseBody, responseObjLikeJqXHR) => {
            if (err) {
              console.log(err);
              return;
            } else if (responseBody) {
