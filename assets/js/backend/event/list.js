class IndexListEventBackendEKP extends BaseBackendEKP {
  constructor() {
    super();
    this.initialize();
  }
  ...
  initialize() {
    ...
    this.list = new ListIndexEventBackendEKP(this);
  }
}

class ListIndexEventBackendEKP {
  constructor(opts) {
    _.extend(this, opts);
  ...
  initDataTable() {
    ...
    var table = this.tableObj.DataTable({
      ...
      "processing": true,
      "serverSide": true,
      "ajax": "/api/v1/backend/event/search?status=" + params.status,
      ...
      columnDefs: [
        { "orderable": false, "targets": [0, 1, -1, -2, ] }
      ],
      "order": [],
      "iDisplayLength": 10,
  ...
  initSweetAlert(id) {
    swal({
      ...
      if (value) {
        //get AJAX data
        Cloud.deleteEvent.with({ ids: id,  _csrf: window.SAILS_LOCALS._csrf }).protocol('jQuery').exec((err, responseBody, responseObjLikeJqXHR) => {
          if (err) {
            console.log(err);
            return;
	...
	initSwitchStatus() {
        ...
        Cloud.switchStatusEvent.with({ id: id,  _csrf: window.SAILS_LOCALS._csrf }).protocol('jQuery').exec((err, responseBody, responseObjLikeJqXHR) => {
					if (err) {
						console.log(err);
						return;
					} else if (responseBody) {
