...
class ListFeeInvoiceBackendEKP {
  ...
  initSweetAlertDeleteFeeInvoice(id) {
    ...
    swal({
      ...
      if (value) {
        //get AJAX data
        Cloud.deleteFeeInvoice.with({ ids: id,  _csrf: window.SAILS_LOCALS._csrf }).protocol('jQuery').exec((err, responseBody, responseObjLikeJqXHR) => {
          if (err) {
            console.log(err);
            return;
  ...
  initSweetAlertPublicFeeInvoice() {
    $('#btnPublicFeeInvoice').click(() => {
      ...
      swal({
        ...
        if (value) {
          //get AJAX data
          Cloud.publicFeeInvoice.with({ ids: _this.feeInvoiceIds,  _csrf: window.SAILS_LOCALS._csrf }).protocol('jQuery').exec((err, responseBody, responseObjLikeJqXHR) => {
            if (err) {
              console.log(err);
              return;
            } else if (responseBody) {
