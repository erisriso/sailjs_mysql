...
class ListIndexAttendentBackendEKP {
  ...
  handleItemActions() {
    ...
    $('#btnUpdateAttendent').click(() => {
      ...
      params._csrf = window.SAILS_LOCALS._csrf;
      if (!params.time) {
    ...
    _this.tableObjV1.on('change', '.checkIn', function (e) {
      ...
      Cloud.checkInAttendent.with({id:attendentId, _csrf: window.SAILS_LOCALS._csrf }).protocol('jQuery').exec((err, responseBody, responseObjLikeJqXHR) => {
        if (err) {
          if (responseBody.message) {
            _this.alert.removeClass('d-none').addClass("alert-warning").html(responseBody.message);
            setTimeout(function(){
